...
void gpioin(cJSON *sensor_json)
{
    ...
    while (item != NULL)
    {
        ...
        if (cJSON_IsString(sensor_) && cJSON_IsString(id_) && cJSON_IsNumber(pin_) && cJSON_IsNumber(int_) && cJSON_IsNumber(ep_) && cJSON_IsString(cluster_))
        {
            ...
            if ((strcmp(cluster, "Contact") == 0 || strcmp(cluster, "ZoneStatus") || strcmp(cluster, "Motion") || strcmp(cluster, "WaterLeak") || strcmp(cluster, "Door_Window") || strcmp(cluster, "Fire") || strcmp(cluster, "Occupancy") || strcmp(cluster, "Carbon") || strcmp(cluster, "Remote_Control") || strcmp(cluster, "BINARY")) && strcmp(sensor, "gpioIN") == 0)
            {
